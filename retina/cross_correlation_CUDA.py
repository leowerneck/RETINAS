...
lib = ctypes.CDLL(os.path.join(".","cross_correlation_CUDA","libimageanalysis.so"))
...
#                                   const REAL A0,
#                                   const REAL B1 );
lib.initialize_CUDAstate.argtypes = [c_int, c_int, c_float, c_float, c_float]
...
def initialize_cross_correlation(N_horizontal, N_vertical, time_constant, upsample_factor, **kwargs):
    ...
    state.CUDAstate = lib.initialize_CUDAstate(N_horizontal, N_vertical, upsample_factor, state.A0, state.B1)
