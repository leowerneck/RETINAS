...
lib = ctypes.CDLL(os.path.join(".","cross_correlation_C","libimageanalysis.so"))
...
# Step 4.b: initialize_Cstate
# Cstate_struct *initialize_Cstate( const int N_horizontal,
#                                   const int N_vertical,
#                                   const REAL upsample_factor,
#                                   const REAL A0,
#                                   const REAL B1 );
lib.initialize_Cstate.argtypes = [c_int, c_int, c_float, c_float, c_float]
lib.initialize_Cstate.restype  = c_void_p

# Step 4.c: finalize_Cstate
# int finalize_Cstate( Cstate_struct *restrict Cstate );
lib.finalize_Cstate.argtypes = [c_void_p]
lib.finalize_Cstate.restype  = c_int

# Step 4.d: typecast_and_sum_image
...
def dump_eigenframe(state):
    err = lib.dump_eigenframe(state.Cstate)
...
def preprocess_cross_correlation_data(state,image_array):
    ...
    # Step 6.c: The image is now in State.Cstate.new_image_time_domain,
...
def preprocess_cross_correlation_data_4x4_rebin(state,image_array):
    ...
    # Step 7.c: The image is now in State.Cstate.new_image_time_domain,
    #           which can only be accessed in the C code.
    return brightness
...
def initialize_cross_correlation(N_horizontal, N_vertical, time_constant, upsample_factor, **kwargs):
    ...
    # Step 8.d: Set the C state using the initialize_Cstate() function
    state.Cstate = lib.initialize_Cstate(N_horizontal, N_vertical, upsample_factor, state.A0, state.B1)
...
def analyze_cross_correlation(state):
    ...
    """
    # Step 9.a: Initialize the displacements to zero
    displacements = np.zeros(2,dtype=np.float32)
    ...
    if state.is_first_frame:
        ...
        key_err = lib.set_zeroth_eigenframe(state.Cstate)
        ...
        key_err         = lib.cross_correlate_and_build_next_eigenframe(state.Cstate,
...
#          the Cstate object, freeing memory in the process
def terminate_cross_correlation(state):
    key_err = lib.finalize_Cstate(state.Cstate)
    if key_err != 0:
