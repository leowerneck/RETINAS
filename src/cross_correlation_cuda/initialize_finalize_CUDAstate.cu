...
CUDAstate_struct *initialize_CUDAstate( const int N_horizontal,
  ...
  cudaMalloc(&CUDAstate->new_image_time_domain ,sizeof(COMPLEX)*NhNv);
  cudaMalloc(&CUDAstate->new_image_freq_domain ,sizeof(COMPLEX)*NhNv);
  cudaMalloc(&CUDAstate->eigenframe_freq_domain,sizeof(COMPLEX)*NhNv);
...
int finalize_CUDAstate( CUDAstate_struct *restrict CUDAstate ) {
  ...
  cudaFree(CUDAstate->new_image_time_domain);
  cudaFree(CUDAstate->new_image_freq_domain);
  cudaFree(CUDAstate->eigenframe_freq_domain);
