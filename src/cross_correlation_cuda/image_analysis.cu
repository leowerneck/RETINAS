...
int dump_eigenframe( CUDAstate_struct *restrict CUDAstate ) {
  ...
  FILE *fp = fopen("eigenframe_cross_correlation_CUDA.txt", "w");
  for(int i=0;i<Nh;i++) {
    for(int j=0;j<Nv;j++) {
...
int cross_correlate_and_build_next_eigenframe(CUDAstate_struct *restrict CUDAstate,
  ...
  element_wise_multiplication_conj(Nh,Nv,NhNv,CUDAstate->new_image_freq_domain,
                                   CUDAstate->eigenframe_freq_domain,
                                   CUDAstate->aux_array1);
  ...
  const int idx_max = find_maxima(CUDAstate->cublasHandle,NhNv,CUDAstate->aux_array_real);
